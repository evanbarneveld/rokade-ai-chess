...
pub fn move_knight(game_state: &mut GameState, piece: Piece, from: (usize, usize), to: (usize, usize), is_capture: bool, promotion_piece: Option<Piece>) -> bool {
    // Perform the actual move on the board
    let moved = game_state.move_piece(from, to);
    if !moved { return false; }

    // Knight move clears any en-passant target
