...
pub fn generate_move_as_san(mode: SearchMode, game_state: GameState, history: &History, search_depth: usize, move_time_in_ms: usize, playing_strength: usize) -> Option<String> {
    ...
    let generated_move = find_best_move_with_mode(mode, &game_state, history, search_depth, playing_strength).map(|(from, to, promo, _score_cp, _depth_used)| (from, to, promo));
    clear_time_budget();
