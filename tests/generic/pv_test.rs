...
use chess::search::{InfoCb, SearchContext, SearchMode};
...
fn depth2_pv_first_move_is_exd5() {
    ...
    let info_cb: Arc<InfoCb> = Arc::new(move |_mv, _sc, _depth, pv, _hf| {
        *last_pv_clone.lock().unwrap() = pv; // capture PV from the most recent iteration
    });
    set_info_callback(&ctx, Some(info_cb));

    let history = game.get_history().clone();
    let depth = 2usize;
    let _san = generate_move_as_san(
        &ctx,
        SearchMode::Normal,
        game.get_game_state(),
        &history,
